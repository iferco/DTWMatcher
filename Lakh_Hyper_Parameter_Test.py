...
audio_folder='./lakh_data/synthesized_midi'
frame_size=[1024]
hop_size=[1024]
name='results_lakh_hyperparameters_cens.txt'
pickle_name=name.split(".")[0]
mode='cens'

def test_base_dtw(duration,mode='chroma', hop_size=256, frame_size=512, iterations=100,sr=44100):
        length=[]
        position_list=[]
        chosen_id=[]
        for i in range(iterations):
            #print("Testing for duration of: ", duration)
            input_file_path = random_lakh_song()
            ...
            input_audio, sr = librosa.load(input_file_path, sr=sr)
            if i == 0:
                ...
                database = create_lakh_database(My_DTW_instance,audio_folder,duration=duration, mode=mode, n_fft=frame_size, stride=hop_size, sr=sr)
            ...
            input_features = My_DTW_instance.extract_features(input_audio, sr=sr, mode=mode, n_fft=frame_size, stride=hop_size)
