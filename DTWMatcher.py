...
class DTWMatcher:
    ...
    def compute_dtw(self, audio_features, score_features, hpcp=False):
        """
        ...
        """
        if hpcp == True:
            D, wp = librosa.sequence.dtw(X=audio_features.T, Y=score_features.T, metric='euclidean')
        else:
    ...
    def identify_score(self, audio_frames, mode='chroma'):
        """
...
def create_lakh_database(MatcherInstance, audio_folder, duration=None, n_fft=4096, stride=512, mode='chroma',sr = 44100):
    """
    Creates a dictionary to store the hashed chroma features of the database.
    ...
    Params:
        ...
        duration: Duration of the audio files wanted to be loaded.
    Returns:
        features_dict: Dictionary containing the features of the database.
